...
function runSelfTests(){
  try{
    ...
    // Additional tests
    console.assert(shortName("Intel Compute Node") === "I CN", "shortName should compress 'Intel Compute Node' to 'I CN'");
    console.assert(shortName("Mgmt Switch") === "Mgmt Sw", "shortName should compress 'Switch' to 'Sw'");

    // placementValid tests
    const itemsPV = [{ id: 'a', name: 'KVM', span: 2, startU: 1 }];
    console.assert(placementValid(itemsPV as any, -1, 3, 2) === true, "placementValid should allow non-overlapping placement");
    console.assert(placementValid(itemsPV as any, -1, 2, 2) === false, "placementValid should reject overlapping placement");
    console.assert(placementValid(itemsPV as any, -1, 0, 1) === false, "placementValid should reject out-of-bounds (low)");

    // materializeCountsToLayout capacity clamp
    const many = materializeCountsToLayout({ KVM: 50 });
    console.assert(many.length <= RACK_U, "materializeCountsToLayout should not exceed rack height");
    const top = many[many.length-1];
    console.assert(top.startU + top.span - 1 <= RACK_U, "last item must fit within rack bounds");
  }catch(e){
...
export default function InteractiveRackPowerGrid(){
  ...
  const [countsByRack, setCountsByRack] = useState<Record<string, Record<string, number>>>(()=> ({
    "Rack 1": {
      ...
      "Core Switch": 1,
      "Mgmt Switch": 1,
      "Intel Control Plane Node": 2,
      "Intel Compute Node": 2,
      "Intel H100 Node": 6,
    },
    "Rack 2": {},
    "Rack 3": {},
...
function RackVisuals({ racks, countsByRack, capacityW, onShowDetails }:{
  ...
  // Hover helpers
  const onBoundaryOver = (rack:string, boundary:number)=>(e:React.DragEvent)=>{ e.preventDefault(); setHoverByRack(p=>({...p, [rack]: boundary})); };
  const clearHover = (rack:string)=>(e?:React.DragEvent)=>{ if(e) e.preventDefault(); setHoverByRack(p=>({...p, [rack]: null})); };
  ...
  const onBoundaryDrop = (rack:string, boundary:number)=>(e:React.DragEvent)=>{
    ...
    try{
      const { rack: r0, id } = JSON.parse(data);
      if (r0 !== rack) return;
      setLayouts(prev=>{
        ...
        // Find valid position if current position conflicts
        if (!placementValid(cur, idx, startU, span)) {
          // Try positions near the target
          for (let offset = 0; offset <= Math.max(startU, RACK_U - startU); offset++) {
            if (offset === 0) continue; // already tried exact position

            // Try above target position
          ...
          <div key={r} className="border rounded-xl p-3">
            ...
            <div className="flex gap-3">
              <div className="relative" style={{width:140}}>
                  ...
                  {layoutItems.map((it)=>{
                    ...
                    return (
                      <div
                        key={it.id}
                        draggable
                        onDragStart={onItemDragStart(r, it.id)}
                        onDragEnd={()=>{ setIsDragging(false); clearHover(r)(); }}
                        className={`ml-1 mr-1 rounded-md border text-[10px] flex items-center justify-between px-1 ${CATEGORY_COLORS[category] || 'bg-gray-100 text-gray-800'}`}
                        style={{ position:'absolute', bottom:(it.startU-1)*16, height:it.span*16, left:0, right:0, opacity:0.95, cursor:'grab' }}
                        title={`${it.name} • ${it.span}U • ${spec?.powerW ?? 0} W`}
                        onClick={()=> onShowDetails(it.name)}
                      >
                        <span className="truncate">{shortName(it.name)}</span>
                        <span>{it.span}U</span>
                      </div>
                    );
                  })}
                ...
                {/* Drop zones at each U slot — render only while dragging */}
                {isDragging && (
                  <div className="absolute inset-0 left-0 top-0" style={{ zIndex: 5 }}>
                    {Array.from({length:RACK_U}).map((_,u)=> (
                      <div key={u}
                        onDragOver={onBoundaryOver(r, u + 1)}
                        onDrop={onBoundaryDrop(r, u + 1)}
                        onDragLeave={clearHover(r)}
                        style={{position:'absolute', left:0, right:0, bottom: u*16, height: 16}}
                        className="border border-dashed border-blue-300 bg-blue-50 bg-opacity-20 hover:bg-blue-100 hover:bg-opacity-50 hover:border-blue-500"
                      />
                    ))}
                  </div>
                )}
